...
int main(int argc, char **argv)
{
  ...
  UnderwaterBranch::readProblem(map_file, currProblem);
  bool useMacroBool = true;
  if (useMacro != 1) useMacroBool = false;
  
  // HUY note - 2. Initialize UnderwaterBranch
  UnderwaterBranch currModel(currProblem, useMacroBool);
  ...
  ParticlesBeliefSet currSet(currModel, currRandSource, numNextBeliefStreams, maxSimulLength);

  // HUY note - 5.
  long obsGrp = 0;
  Obs obs(currModel.getNumObsVar(),0);
  obs[1] = -1;
  vector<long> pathLength;
  pathLength.resize(currProblem.initialBeliefStates.size(), 0);
  Belief *root = currSet.beliefFromStateSet(currProblem.initialBeliefStates,obs,pathLength);
  ...
  Bounds bounds(currModel, policyGraph, currSim, currRandSource, numBackUpStreams, maxSimulLength, maxSimulLength);
