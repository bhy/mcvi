...
int main(int argc, char **argv)
{
  ...
  UnderwaterBranch currModel(currProblem, true);
  ...
  if (numTrials == 1){
    for(long k=0;k<currProblem.initialBeliefStates.size();k++){
      trace.clear();
      actTrace.clear();
      currSim.runSingle(maxSimulLength, avgReward, avgDiscounted, currProblem.initialBeliefStates[k], trace, actTrace, true, currRandSource);
	...
	for (long i= 0; i<numTrials; i++){
      ...
      currSim.runSingle(maxSimulLength, avgReward, avgDiscounted, currProblem.initialBeliefStates[init], trace, actTrace, false, currRandSource);
      sumReward += avgReward;
